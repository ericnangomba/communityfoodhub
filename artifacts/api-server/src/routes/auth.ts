...
router.post("/auth/register", async (req, res, next) => {
  try {
    ...
    if (!hasDatabase) {
      ...
      res.status(201).json({ user: { id: user.id, name: user.name, fullName: user.fullName, email: user.email, phoneNumber: user.phoneNumber, role: user.role, hubId: user.hubId } });
      return;
    }
...
router.post("/auth/login", async (req, res, next) => {
  try {
    ...
    if (!user || !verifyPassword(input.password, user.passwordHash)) {
      if (!hasDatabase) {
        ...
        if (fallbackUser && verifyPassword(input.password, fallbackUser.passwordHash)) {
          ...
          res.cookie(SESSION_COOKIE, session.id, cookieOptions(session.expiresAt));
          res.json({ user: { id: fallbackUser.id, name: fallbackUser.name, fullName: fallbackUser.fullName, email: fallbackUser.email, phoneNumber: fallbackUser.phoneNumber, role: fallbackUser.role, hubId: fallbackUser.hubId } });
          return;
        }
      }
    ...
    const session = await createSession(user.id);
    res.cookie(SESSION_COOKIE, session.id, cookieOptions(session.expiresAt));
    res.json({ user: { id: user.id, name: user.name, fullName: user.fullName, email: user.email, phoneNumber: user.phoneNumber, role: user.role, hubId: user.hubId } });
  } catch (error) {
    next(error);
  }
});
...
router.get("/auth/users", requireAuth, requireRoles("SUPER_ADMIN"), async (_req, res, next) => {
  try {
    if (!hasDatabase) {
      res.json({ users: [...fallbackUsers.values()].map((user) => ({ id: user.id, name: user.name, fullName: user.fullName, email: user.email, phoneNumber: user.phoneNumber, role: user.role, hubId: user.hubId })) });
      return;
    }
